    ...
    <Card className="w-full max-w-2xl mx-auto animate-fade-in hover-lift">
      ...
      <CardContent className="space-y-4">
        {items.map((item) => {
          ...
          const effectivePrice = item.product.price + priceAdjustment;
          const maxStock = item.variant ? item.variant.stock_quantity : item.product.stock_quantity;
            ...
            <div key={`${item.id}`} className="flex items-center gap-4 p-4 border rounded-lg hover-lift transition-all duration-300">
              ...
              <div className="flex-1">
                ...
                {item.variant && (
                  <div className="flex gap-2 mt-1">
                    {item.variant.size && (
                      <Badge variant="outline" className="text-xs">Size: {item.variant.size}</Badge>
                    )}
                    {item.variant.color && (
                      <Badge variant="outline" className="text-xs">Color: {item.variant.color}</Badge>
                    )}
                  </div>
                )}
              ...
              <div className="text-right">
                <p className="font-semibold">
                  KSH {(effectivePrice * item.quantity).toFixed(2)}
                </p>
                <Button
                  variant="ghost"
                  size="sm"
